...
int main() 
{
	...
	int count;
	if ((count = recv(c_sock, (void *)&data, sizeof(struct node), 0)) == -1) {
		error("recieving.");
	}

	printf("recieved: %c %d %f\n", data.ch, data.in, data.fl);
	
	data.ch -= 1;
	data.in -= 200;
	data.fl /= 5376;

	if ((count = send(m_sock, (void*)&data, sizeof(struct node), 0)) == -1) {
		error("sending.");
	}
	printf("sent: %c %d %f\n", data.ch, data.in, data.fl);
